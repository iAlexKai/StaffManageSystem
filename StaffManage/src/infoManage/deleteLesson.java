...
public class deleteLesson extends JFrame implements ActionListener{
	JLabel JL = new JLabel("删除培训信息");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == JBDelete) {
			...
			try {
				...
				System.out.println("Successfully connected to the database lesson_info");
				// get the data
				...
				// 如果有结果，则给出提示，否则写入数据库
				if (!result_set.next()) 
					JOptionPane.showMessageDialog(null, "找不到此课程数据，删除失败，请重新输入！");
				else {
					sql = "delete from lesson_info where tid = '"+sSid+"'";  // 注意 delete语句里面没有 *！！！！！
					// 执行sql语句，并获得返回值
