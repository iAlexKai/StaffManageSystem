...
public class Lesson_info extends MainPage {
	...
	// url
	String url = "jdbc:mysql://127.0.0.1:3306/train_info?useSSL=false";  // 注意数据库名字的修改
	// user-name
	String user = "root";
	// pass-word
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == JBSearchId) { 
			...
			try {
				...
				// 如果有结果，则给出提示，否则写入数据库
				if (!result_set.next()) 
					JOptionPane.showMessageDialog(null, "找不到此号对应的课程！");
				else {
		...
		if(e.getSource() == JBSearchName) { 
			...
			sql = "select * from lesson_info where className = '"+sName+"'";
			try {
				Class.forName(jd);
				// connect to database object
				connection = (Connection)DriverManager.getConnection(url, user, password);
				...
				// 如果有结果，则给出提示，否则写入数据库
				if (!result_set.next()) 
					JOptionPane.showMessageDialog(null, "找不到此培训课程！");
				else {
					Vector rows = new Vector();
